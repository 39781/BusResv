            ...
            if (data.payload.buttons) {
                ...
                for (var j = 0; j < data.payload.buttons.length; j++) {
                  if(data.payload.buttons[j].postback=="other_queries_non_invoice"){
                    ...
                    cardButtons += `<button type="button"  class="btn btn-primary infocard-btn-custom webview"  onClick="window.open('https://stark-ocean-67616.herokuapp.com/${data.payload.buttons[j].url}','chat70994705','width=400,height=600,resizable=yes');$('.webview').hide();return false;" >${data.payload.buttons[j].title}</button>`
                  }
